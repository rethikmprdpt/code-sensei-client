...
import { AlertTriangle, CheckCircle, Clock, ShieldAlert, Zap, BookOpen } from 'lucide-react';
  ...
  if (loading) {
    return (
      <div className="h-full flex flex-col items-center justify-center bg-gray-900 text-gray-400 space-y-4">
        ...
        <p className="animate-pulse font-mono text-sm">Parsing CST & Querying Gemini...</p>
      </div>
    );
  }
  ...
  if (!analysis) {
    return (
      <div className="h-full flex flex-col items-center justify-center bg-gray-900 text-gray-500 p-8 text-center">
        ...
        <h3 className="text-lg font-medium text-gray-300">Code Sensei is Ready</h3>
        <p className="text-sm mt-2 max-w-xs mx-auto">
          Paste your Python code on the left and hit "Analyze" to detect bad logic.
        </p>
      </div>
    );
  }
            ...
            >
              ...
              <div className="bg-gray-800 p-4 border-b border-gray-700 flex justify-between items-center">
                ...
                <div className={`px-3 py-1 rounded-full text-xs font-bold border ${
                  report.quality_score >= 8 ? 'bg-green-500/10 text-green-400 border-green-500/20' :
                  report.quality_score >= 5 ? 'bg-yellow-500/10 text-yellow-400 border-yellow-500/20' :
              ...
              <div className="p-5 space-y-6">
                ...
                <div>
                  <h4 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">Logic Summary</h4>
                ...
                <div className="grid grid-cols-1 gap-4">
                  ...
                  <div className="flex items-center gap-3 bg-purple-500/5 p-3 rounded-lg border border-purple-500/10">
                    ...
                    <div>
                      <div className="text-xs text-purple-300/70 uppercase font-bold">Time Complexity</div>
                      <div className="font-mono text-purple-300 font-bold">{report.complexity_estimate}</div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-3">
                    {report.issues.length === 0 ? (
                      <div className="flex items-center gap-2 text-green-400 text-sm bg-green-500/5 p-3 rounded border border-green-500/10">
                        <CheckCircle className="w-4 h-4" /> Clean code! No major issues found.
                      </div>
                    ) : (
                        ...
                        <div key={i} className="bg-red-500/5 border border-red-500/20 rounded-lg p-4">
                          <div className="flex justify-between items-start mb-2">
                            <span className="text-red-300 font-semibold text-sm flex items-center gap-2">
                              <AlertTriangle className="w-4 h-4" /> {issue.issue_type}
                            </span>
                          ...
                          {issue.fix_suggestion && (
                            <div className="mt-2 relative group">
                              ...
                              <div className="pl-3">
                                <p className="text-[10px] text-gray-500 mb-1 uppercase font-bold">Suggested Fix</p>
